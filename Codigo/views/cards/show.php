                ...
                <div class="card-stats-grid">
                    ...
                    <?php if (!empty($card['stage'])): ?>
                        <div class="stat-box">
                            ...
                            <span class="stat-value"><?= htmlspecialchars($card['stage']) ?></span>
                        </div>
                    <?php endif; ?>
                    <?php if (!empty($card['evolveFrom'])): ?>
                        <div class="stat-box">
                            <span class="stat-label">Evoluciona de</span>
                            <span class="stat-value"><?= htmlspecialchars($card['evolveFrom']) ?></span>
                        </div>
                    <?php endif; ?>
                    <?php if (!empty($card['retreat'])): ?>
                        <div class="stat-box">
                            <span class="stat-label">Retirada</span>
                            <span class="stat-value"><?= $card['retreat'] ?> ⚪</span>
                        </div>
                    <?php endif; ?>
                    <?php if (!empty($card['illustrator'])): ?>
                        <div class="stat-box">
                            <span class="stat-label">Ilustrador</span>
                            <span class="stat-value"><?= htmlspecialchars($card['illustrator']) ?></span>
                        </div>
                    <?php endif; ?>
                </div>
                ...
                <?php if (!empty($card['pricing']['cardmarket'])): ?>
                    ...
                    <div class="price-box">
                        ...
                        <div class="price-grid">
                            <?php if (!empty($price['avg'])): ?>
                                <div class="price-item">
                                    <span>Precio medio</span>
                                    <strong><?= number_format($price['avg'], 2) ?> €</strong>
                                </div>
                            <?php endif; ?>
                            <?php if (!empty($price['low'])): ?>
                                <div class="price-item">
                                    <span>Precio mínimo</span>
                                    <strong><?= number_format($price['low'], 2) ?> €</strong>
                                </div>
                            <?php endif; ?>
                            <?php if (!empty($price['trend'])): ?>
                                <div class="price-item">
                                    <span>Tendencia</span>
                                    <strong><?= number_format($price['trend'], 2) ?> €</strong>
                                </div>
                            <?php endif; ?>
                            <?php if (!empty($price['avg7'])): ?>
                                <div class="price-item">
                                    <span>Media 7 días</span>
                                    <strong><?= number_format($price['avg7'], 2) ?> €</strong>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
                                ...
                                <div class="attack-header">
                                    <div class="attack-cost">
                                        <?php foreach ($attack['cost'] ?? [] as $cost): ?>
                                            <span class="cost-badge" style="background:<?= $typeColors[$cost] ?? '#ccc' ?>33; color:<?= $typeColors[$cost] ?? '#888' ?>">
